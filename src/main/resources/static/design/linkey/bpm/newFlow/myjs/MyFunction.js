...
function saveObj(obj) {
	var d = {};
	var p = parent.GetUrlArg("Processid")
	...
	$.ajax({
	    ...
	    url: CONFIG.url.saveFlowChartGraphic + "?Processid="+p,
	    headers: {
...
function deleteNodeFromDB(idArr) {
	...
	var processId = parent.GetUrlArg("Processid"), i;
	for (i = 0; i < idArr.length; i++) {
		...
		conn.send();*/
		$.ajax({
		    url: CONFIG.url.deleteNode + "&Processid=" + processId + "&Nodeid=" + getRemovePrefixId(idArr[i]),
		    headers: {
		    	sysid: CONFIG.rest.sysid,
		    	syspwd: CONFIG.rest.syspwd,
		    	userId: CONFIG.rest.userId
...
function saveAllDefaultNode() {
	var sendDataArr = [];
	var processId = parent.GetUrlArg("Processid");

	var s =  CONFIG.url.saveAllDefaultNode + '&Processid=' + processId + '&Nodeid=';
...
function SetProperty(a, b) {
	...
	var d = parent.GetUrlArg("Processid");
	...
	var l = "./"+c+".html" + "?Processid=" + d + "&Nodeid=" + k + "&ExtNodeType=" + c + "&WF_Appid=" + top.GetUrlArg("WF_Appid");
	if(b == "Router") {
		l += "&SourceNode=" + $('#' + a).attr('sourceid') + "&TargetNode=" + $('#' + a).attr('targetid');
	}
	var m = 'dialogWidth:780px;dialogHeight:600px;dialogLeft:' + i + ';dialogTop:' + j + ';directories:no; localtion:no; menubar:no; status=no; toolbar=no;scrollbars:no;Resizeable=no;help:0;';
